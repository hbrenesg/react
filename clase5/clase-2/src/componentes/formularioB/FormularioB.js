...
class FormularioB extends React.Component {
    ...
    ejecutarCambios(event) {
        const target = event.target; //Tipo de input
        const value = target.type === 'checkbox' ? target.checked : target.value; //Verificación si es un input checkbox o number
    ...
    render() {
        return (
            <form>
                ...
                <br />
                {this.state.estanIngresando + " | " + this.state.numeroPasajeros}
            </form>
        );
    }
}
